          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              <h3 className="text-sm font-bold text-white flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded bg-indigo-500/20 text-indigo-300 font-mono text-[9px] font-bold">3 Presets</span>
              </h3>
              <p className="text-[11px] text-slate-400">Select the route sequence that best fits your client's priorities</p>
            </div>
          </div>
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-2xl border transition-all flex flex-col justify-between space-y-4 relative ${
                    isPurple
                      ? 'bg-gradient-to-b from-purple-950/40 to-slate-900 border-purple-500/50 shadow-purple-500/10 shadow-xl'
                      : isEmerald
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1">
                      <span className={`px-2 py-0.5 rounded font-extrabold text-[10px] uppercase tracking-wider inline-block ${
                        isPurple ? 'bg-purple-500/20 text-purple-300 border border-purple-500/30' :
                        isEmerald ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30' :
                        'bg-amber-500/20 text-amber-300 border border-amber-500/30'
                      }`}>
                        {opt.badgeText}
                      </span>
                      <h4 className="font-extrabold text-white text-sm leading-tight">{opt.name}</h4>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[11px] font-medium pt-1">
                      {/* Replaced "Total Listings" with "Covered in Tour Window" as requested */}
                      <div className="p-2 rounded-lg bg-slate-950 border border-slate-800 space-y-0.5">
                        <div className="text-slate-400 text-[10px] flex items-center gap-1">
                          <CheckCircle2 className="w-3 h-3 text-emerald-400" /> Covered in Window
                        </div>
                      ...
                      <div className="p-2 rounded-lg bg-slate-950 border border-slate-800 space-y-0.5">
                        <div className="text-slate-400 text-[10px] flex items-center gap-1">
                          <Star className="w-3 h-3 text-amber-400" /> Must See
                        </div>
                      ...
                      <div className="p-2 rounded-lg bg-slate-950 border border-slate-800 space-y-0.5 col-span-2 flex items-center justify-between">
                        <span className="text-slate-400 text-[10px] flex items-center gap-1">
                          <Car className="w-3 h-3 text-indigo-400" /> Total Drive Metrics
                        </span>
                    ...
                    <div className="space-y-1.5 pt-1">
                      <div className="text-[10px] uppercase font-bold text-slate-400">Proposed Stop Order:</div>
                      <div className="space-y-1 max-h-36 overflow-y-auto pr-1">
                        {opt.stops.map((s, idx) => (
                          <div key={s.id || idx} className="text-[11px] text-slate-300 flex items-center justify-between p-1.5 rounded bg-slate-950 border border-slate-800/80">
                            <span className="truncate max-w-[180px]">
                              <strong className="text-indigo-400 mr-1.5">#{idx + 1}</strong>
                              {s.normalized_address}
                            </span>
                            {s.priority === 'MUST_SEE' && (
                              <span className="text-[9px] font-bold px-1 rounded bg-amber-500/20 text-amber-300 border border-amber-500/30 shrink-0">
                                Must See
                              </span>
                            )}
                          </div>
