...
export default function MapView({
  ...
  return (
    <div className="relative w-full h-full min-h-[380px] sm:min-h-[450px] bg-slate-100 dark:bg-slate-950 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-xl flex flex-col font-sans">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h4 className="text-xs font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
              ...
              <span className="px-1.5 py-0.5 rounded bg-emerald-100 dark:bg-emerald-500/20 text-emerald-800 dark:text-emerald-300 font-mono text-[9px]">Live Hover Sync</span>
            </h4>
            <p className="text-[10px] text-slate-500 dark:text-slate-400 hidden sm:block">{stops.length} Property Showing Stops · Long Island</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between w-full sm:w-auto gap-2">
          ...
          <div className="flex bg-slate-100 dark:bg-slate-950 p-0.5 rounded-lg border border-slate-200 dark:border-slate-800 text-[10px] sm:text-[11px] font-bold">
            ...
            >
              ...
              <span className="hidden sm:inline">Google Interactive Map</span>
            ...
            >
              ...
              <span className="hidden sm:inline">Google Directions</span>
          ...
          {stops.length > 0 && (
            <a
              ...
              className="px-2.5 py-1 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-[10px] sm:text-[11px] font-bold flex items-center gap-1 shadow transition-colors shrink-0"
            >
      ...
      <div className="px-3 py-1.5 bg-slate-50 dark:bg-slate-900/80 border-b border-slate-200 dark:border-slate-800/80 flex items-center gap-1.5 overflow-x-auto no-scrollbar z-10">
        <span className="text-[10px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wider shrink-0 mr-1 flex items-center gap-1">
          <MapPin className="w-3 h-3 text-indigo-600 dark:text-indigo-400" /> Tour Pins:
        </span>
        {stops.map((stop, idx) => {
              ...
              className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all shrink-0 flex items-center gap-1 cursor-pointer border ${
                isHovered
                  ...
                  : isSelected
                    ...
                    : 'bg-white dark:bg-slate-950 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white'
              }`}
            >
              <span>#{idx + 1}</span>
      ...
      {/* Main Map Body */}
      <div className="relative flex-1 w-full h-full min-h-[350px] bg-slate-100 dark:bg-slate-950">
      ...
      {activePinStop && (
        <div className="p-3 bg-white dark:bg-slate-900/95 backdrop-blur-md border-t border-slate-200 dark:border-slate-800 flex items-center justify-between z-10 shadow-sm">
          <div className="flex items-center space-x-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-[10px] text-slate-500 dark:text-slate-400 flex items-center gap-3">
                <span>Arrival: <strong className="text-indigo-600 dark:text-indigo-300">{activePinStop.planned_arrival}</strong></span>
          ...
          {onRemoveStop && (
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-950 hover:bg-rose-600 text-rose-700 dark:text-rose-300 hover:text-white border border-slate-200 dark:border-slate-800 text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer shrink-0 ml-2"
            >
