...
import { X, Copy, Check, MessageSquare, CheckCircle2, AlertCircle, XCircle, Clock, ExternalLink, Send, Phone } from 'lucide-react';
...
export default function AppointmentModal({
  ...
  React.useEffect(() => {
    if (stop) {
      ...
      setDraftText(renderTemplate(DEFAULT_SMS_TEMPLATE, vars));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fadeIn font-sans">
      <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        {/* Content */}
        <div className="p-6 overflow-y-auto space-y-5 flex-1 bg-white dark:bg-slate-900 text-xs">
          ...
          <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-2">
            ...
            <div className="flex items-center gap-2 bg-white dark:bg-slate-900 px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800">
              ...
              <span className="text-slate-500 dark:text-slate-400 font-semibold shrink-0">Agent Phone #:</span>
              <input
                type="text"
                value={agentPhoneInput}
                onChange={(e) => setAgentPhoneInput(e.target.value)}
                placeholder="e.g. (516) 555-0188"
                className="bg-transparent text-slate-900 dark:text-white font-mono font-bold text-xs focus:outline-none w-full"
              />
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-400">
              <span className="font-semibold text-slate-700 dark:text-slate-300">Ready-to-Send SMS Draft</span>
