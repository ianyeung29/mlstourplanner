    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 py-12 px-4 sm:px-6 lg:px-8 font-sans transition-colors duration-200">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="space-y-8 text-xs text-slate-700 dark:text-slate-300 leading-relaxed">
          {/* Guarantee Banner */}
          <div className="p-4 rounded-2xl bg-emerald-50 dark:bg-gradient-to-r dark:from-emerald-950/60 dark:to-slate-900 border border-emerald-300 dark:border-emerald-500/40 flex items-start gap-3">
            <UserCheck className="w-5 h-5 text-emerald-600 dark:text-emerald-400 shrink-0 mt-0.5" />
            <div>
              <strong className="text-slate-900 dark:text-white text-xs font-bold block mb-1">Zero Data Sale Guarantee</strong>
          ...
          <section className="space-y-3 bg-white dark:bg-slate-900/60 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
              <div className="p-3 rounded-xl bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-1">
                <strong className="text-slate-900 dark:text-white text-[11px] block">Google Maps & Directions API</strong>
                <p className="text-[10px] text-slate-600 dark:text-slate-400">Used for geocoding property addresses, rendering interactive maps, and calculating real-time driving routes.</p>
              </div>
              <div className="p-3 rounded-xl bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-1">
                <strong className="text-slate-900 dark:text-white text-[11px] block">Resend Email API</strong>
                <p className="text-[10px] text-slate-600 dark:text-slate-400">Used to securely deliver styled client itinerary emails and listing agent showing requests.</p>
              </div>
            </div>
          </section>
        ...
        <div className="pt-6 border-t border-slate-200 dark:border-slate-800 flex justify-between text-xs text-slate-600 dark:text-slate-400">
          ...
          <Link href="/terms" className="text-indigo-600 dark:text-indigo-400 hover:underline font-bold">View Terms of Service →</Link>
        </div>
      </div>
    </div>
  );
